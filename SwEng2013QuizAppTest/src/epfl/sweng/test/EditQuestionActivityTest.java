...
public class EditQuestionActivityTest extends
	...
	public void testQuestionEdited() {
		...
		solo.sleep(DODO);
		assertTrue("correct Typo before clicking",
				"✘".equals(correct.getText()) /*
											 * &&
											 * notCorrect.getText().equals("✘")
											 */);

		solo.clickOnView(correct);
		solo.sleep(DODO);
		assertTrue("Correct typo", "✔".equals(correct.getText()) /*
																 * &&
																 * notCorrect.
																 * getText
																 * ().equals
		...
		Log.e("Test", s + " = " + question.getText() + " "
				+ question.getText().equals(s));
		solo.sleep(DODO);
		assertTrue("title edited",
				"test question, please ignore".equals(question.getText()
						.toString()));
		...
		this.mockHttpClient
				.pushCannedResponse(
						...
						HttpStatus.SC_OK, Q.toPostEntity(), "application/json");
