...
public class EditQuestionActivity extends Activity {
	...
	private int auditEditTexts() {
		...
		if (questionField == null
				...
				|| !("Type in the question's text body".equals(questionField
						.getHint()))) {
			editErrors++;
		...
		if (idList.size() != 0) {
			for (int i = 0; i < idList.size(); i++) {
				...
				if (!zeroOrMore
						|| !"Type in the answer".equals(editCheck.getHint())
		...
		if (tagsText == null || tagsText.getVisibility() != View.VISIBLE
				|| !("Type in the question's tags".equals(tagsText.getHint()))) {
			editErrors++;
	...
	private int auditButtons() {
		...
		if (idList.size() != 0) {
			for (int i = 0; i < idList.size(); i++) {
				Button removeCheck = (Button) findViewById(idList.get(i)
						+ removeCst);
				if (!remToAns || !"\u002D".equals(removeCheck.getText())
		...
		if (idList.size() != 0) {
			for (int i = 0; i < idList.size(); i++) {
				...
				if (!togToAns || !(("\u2714".equals(correctCheck.getText())) // correct
						|| ("\u2718".equals(correctCheck.getText()))) // wrong
	...
	private int auditAnswers() {
		...
		for (int i = 0; i < idList.size(); i++) {
			Button correctCheck = (Button) findViewById(idList.get(i));
			if ("\u2714".equals(correctCheck.getText())) {
				correctCount++;
			}
		}
		if (correctCount > 1) {
